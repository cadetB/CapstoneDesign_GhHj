...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO physical_competitions (user_id, competition_name, participation_date, proof_file_path) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isss", $user_id, $competition_name, $participation_date, $proof_file_path);
...
<head>
    ...
    <title>체육 대회 참여 정보 입력</title>
</head>
<body>
    <h2>체육 대회 참여 정보 입력</h2>
    ...
    <?php else: ?>
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
            <label for="competition_name">참여 대회:</label>
